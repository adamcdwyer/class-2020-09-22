...
sep_old <- 
  ...
  filter(u_grad > 10) %>% 
  group_by(id, title) %>% 
  summarize(ugrad_sum = sum(u_grad), .groups = "drop") %>% 
  arrange(title)
...
sep_new <- 
  read_excel("new_data/new_data.xlsx", 
             skip = 3) %>% 
  clean_names() %>% 
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
...
* Make assumptions to limit the data frame
* Throw out duplicate variables
* Remove columns that we are uninterested in
* Define the variables we wish to evaluate (variables of interest), in this model time: sep_old enrollment vs. sep_new enrollment


After we have a model, Temperance suggests we should:

* Acknowledge that this year is very different in terms of college enrollment due to COVID-19, which would affect the internal validity
* Cannot be generalized across different years
* Since the enrollment data does not include LOA population of Harvard, the data set does not include potential outcomes of this demographic of the Harvard population (these students could have been more likely switch courses and adjust the enrollment data)
